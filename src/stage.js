class Stage {
  ...
  generateFloorGraph() {
    ...
    for (var x = 0; x < this.floor_layer.layer.height; x += 1)
      for (var y = 0; y < this.floor_layer.layer.width; y += 1)
        if (this.floor_layer.layer.data[x][y].index != -1)
          this.floor_graph.addVertex(x * this.floor_layer.layer.width + y, {
            centerPosition: {
              y: x * this.floor_layer.layer.baseTileHeight + offset,
              x: y * this.floor_layer.layer.baseTileHeight + offset,
            },
            layer: "Floor",
          });
    ...
    for (var x = 0; x < this.poo_layer.layer.height; x += 1)
      for (var y = 0; y < this.poo_layer.layer.width; y += 1)
        if (this.poo_layer.layer.data[x][y].index != -1)
          this.floor_graph.addVertex(x * this.poo_layer.layer.width + y, {
            centerPosition: {
              y: x * this.poo_layer.layer.baseTileHeight + offset,
              x: y * this.poo_layer.layer.baseTileHeight + offset,
            },
            layer: "Poo",
          });
    ...
    this.floor_graph.adjList.forEach((vertex, id) => {
      ...
      var underVertex = this.floor_graph.getVertex(
        id + this.floor_layer.layer.width
      );
      var underRightVertex = this.floor_graph.getVertex(
        id + this.floor_layer.layer.width + 1
      );
